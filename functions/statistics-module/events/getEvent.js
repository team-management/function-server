...
module.exports = getEvent = async (clientId, filter, res) => {
    //Require mongoose
    ...
    //Starting logic---------------
    log("DEBUG", `Request recieved for searching for event documents:   ${clientId}, ${JSON.stringify(filter)}`);
    ...
    try {
        ...
        Event.find(filter, (err, result) => {
            if (err) {
                log("ERROR", "There has beeing an error trying to get event from database")
                log("ERROR", err);
                db.close();
                res.status(500).json({ msg: "There has beeing an error trying to get event from database" });
            }
