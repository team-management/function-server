...
module.exports = getStatistics = async (clientId, filter, res) => {
    //Require mongoose
    ...
    //Starting logic---------------
    log("DEBUG", `Request recieved for searching for statistic documents:   ${clientId}, ${JSON.stringify(filter)}`);
    ...
    try {
        ...
        Statistic.find(filter, (err, result) => {
            if (err) {
                log("ERROR", "There has beeing an error trying to get statistics from database")
                log("ERROR", err);
                db.close();
